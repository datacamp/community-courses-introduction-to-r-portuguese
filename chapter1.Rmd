...
success_msg("Bom trabalho! Você percebeu que a linguagem R não exibiu no console o código que você digitou para declarar a variável? Isso é porque o código `x <- 42` não gera nenhuma saída (output), uma vez que R acredita que você precisará dessa variável no futuro. Do contrário, você não precisaria armazenar nenhum valor nessa variável, certo? Proceda para o próximo exercício!  Good job! Have you noticed that R does not print the value of a variable to the console when you did the assignment? `x <- 42` did not generate any output, because R assumes that you will be needing this variable in the future. Otherwise you wouldn't have stored the value in a variable in the first place, right? Proceed to the next exercise!")
```
